    ...
    public partial class DataAccess
    {
        ...
        private static void CreateNewDatabaseFile(string path)
        {
            ...
            using (FileStream databaseTo = File.Create(path))
            {
                ...
                Debug.WriteLine(AssemblyInformation.AssemblyFullyQualifiedName());
                AssemblyInformation.This().GetManifestResourceNames().ToList().ForEach(x => Debug.WriteLine(x));

                using (Stream databaseFrom = AssemblyInformation.This().GetManifestResourceStream("BookMan.Useful.Data.BlankAccessDatabase_v11.accdb"))
                // using (Stream databaseFrom = AssemblyInformation.This().GetManifestResourceStream(string.Format(CultureInfo.InvariantCulture, "{0}.Useful.Data.BlankAccessDatabase_v11.accdb", AssemblyInformation.AssemblyName())))
                {
        ...
        private static void ExecuteSqlFile(DbTransaction transaction, string filename)
        {
            ...
            using (Stream s = AssemblyInformation.This().GetManifestResourceStream(string.Format(CultureInfo.InvariantCulture, "BookMan.Useful.Data.Script.{0}", filename)))
